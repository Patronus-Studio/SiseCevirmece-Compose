...
class PackageViewModel @Inject constructor(
    ...
    suspend fun getPackageCategories() {
        ...
        _categories.value = (categories.body()?.packageCategoryModel ?: arrayListOf()) as MutableList<PackageCategoryModel>
        _isLoading.value = false
    }

    fun clickedBtn(clickedItemId: Int) {
        val findedActiveBtnIndex = _categories.value.indexOfFirst {
            it.isSelected == YESORNO.YES
        }
        ...
        _categories.value.forEach {
            if(findedActiveBtnIndex == it.id -1) tempList.add(it.copy(isSelected = YESORNO.NO))
            else if (clickedItemId == it.id -1) tempList.add(it.copy(isSelected = YESORNO.YES))
